...
let ExifTags = {
	...
	0xA20E: 'FocalPlaneXReoffer', // Number of pixels in width direction per FocalPlaneReofferUnit
	0xA20F: 'FocalPlaneYReoffer', // Number of pixels in height direction per FocalPlaneReofferUnit
	0xA210: 'FocalPlaneReofferUnit', // Unit for measuring FocalPlaneXReoffer and FocalPlaneYReoffer
	...
	if (img.src) {
		if (/^data:/i.test(img.src)) {
			...
			fileReader.onload = function (event) {
				handleBinaryFile(event.target.result);
			};
      ...
      if (debug) console.log('Got file of length ' + event.target.result.byteLength)
			handleBinaryFile(event.target.result);
		};
	...
	switch (type) {
		...
		case 5: // rational = two long values, first is numerator, second is denominator
			if (numValues == 1) {
				...
				if (!val.numerator) {
					alert('Error occured with this image')
				}
